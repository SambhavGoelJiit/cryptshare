import type { Block as IPLDBlock, Version as LinkVersion } from 'multiformats';
import type { Phantom, Await } from '@ipld/dag-ucan';
import * as UCAN from '@ipld/dag-ucan';
import type { Capability, ServiceInvocation, InferReceipts, InferInvocations, Receipt, ByteView, Invocation, AgentMessage } from './lib.js';
/**
...
export interface Channel<S extends Record<string, any>> extends Phantom<S> {
    request<I extends Tuple<ServiceInvocation<Capability, S>>>(request: HTTPRequest<AgentMessage<{
        In: InferInvocations<I>;
        Out: Tuple<Receipt>;
    }>>): Await<HTTPResponse<AgentMessage<{
        Out: InferReceipts<I, S>;
        In: Tuple<Invocation>;
    }>>>;
}
export interface RequestEncoder {
    encode<T extends AgentMessage>(message: T, options?: RequestEncodeOptions): Await<HTTPRequest<T>>;
}
...
export interface ResponseEncoder {
    encode<T extends AgentMessage>(message: T, options?: EncodeOptions): Await<HTTPResponse<T>>;
}
 ...
 */
export interface Block<T extends unknown = unknown, Format extends number = number, Alg extends number = number, V extends LinkVersion = 1> extends IPLDBlock<T, Format, Alg, V> {
    data?: T;
}
