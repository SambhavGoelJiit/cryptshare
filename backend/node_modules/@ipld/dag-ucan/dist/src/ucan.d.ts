import type { MultibaseEncoder, MultibaseDecoder, MultihashDigest, MultihashHasher, Block as IPLDBlock, Link as IPLDLink, Version as LinkVersion, Phantom, ByteView } from "multiformats";
...
export type Code = MulticodecCode<typeof CBOR_CODE, "CBOR"> | MulticodecCode<typeof RAW_CODE, "Raw">;
/**
 ...
 */
export interface PrincipalView<ID extends DID = DID> extends ByteView<Principal<ID>>, Principal<ID> {
}
/**
 * Entity that can verify UCAN signatures against a {@link Principal} produced
 * with the algorithm `A` (see {@link Crypto.Verifier}).
 */
export interface Verifier<ID extends DID = DID, SigAlg extends Crypto.SigAlg = Crypto.SigAlg> extends Crypto.Verifier<SigAlg>, Principal<ID> {
}
 ...
 * signing algorithm A
 */
export interface Signer<ID extends DID = DID, SigAlg extends Crypto.SigAlg = Crypto.SigAlg> extends Crypto.Signer<SigAlg>, Principal<ID> {
    /**
 ...
 */
export type JWT<C extends Capabilities = Capabilities> = ToString<UCAN<C>, `${ToString<JWTHeader>}.${ToString<JWTPayload<C>>}.${ToString<Crypto.Signature>}`>;
 ...
 */
export interface Model<C extends Capabilities = Capabilities> extends Header, Payload<C> {
    s: Crypto.SignatureView;
}
export type UCANJSON<T extends UCAN = UCAN> = ToJSON<T, {
    v: Version;
    iss: DID;
    aud: DID;
...
export interface FromJWT<C extends Capabilities = Capabilities> extends Model<C> {
    jwt: JWT<C>;
}
export interface FromModel<C extends Capabilities = Capabilities> extends Model<C> {
    jwt?: never;
}
/**
 * A signed UCAN in either IPLD or JWT format.
 */
export type UCAN<C extends Capabilities = Capabilities> = FromJWT<C> | FromModel<C>;
/**
 ...
 */
export interface UCANOptions<C extends Capabilities = Capabilities, SigAlg extends Crypto.SigAlg = Crypto.SigAlg> {
 ...
 */
export interface Link<C extends Capabilities = Capabilities, Encoding extends MulticodecCode = MulticodecCode, SigAlg extends Crypto.SigAlg = Crypto.SigAlg> extends IPLDLink<UCAN<C>, Encoding, SigAlg> {
}
/**
 ...
 */
export interface Block<C extends Capabilities = Capabilities, Encoding extends MulticodecCode = MulticodecCode, SigAlg extends Crypto.SigAlg = Crypto.SigAlg> extends IPLDBlock<UCAN<C>, Encoding, SigAlg> {
    data?: UCAN<C>;
}
/**
 ...
 *
 */
export interface Capability<Can extends Ability = Ability, With extends Resource = Resource, Caveats extends unknown = unknown> {
    with: With;
    can: Can;
    nb?: Caveats;
}
 ...
 * JSON representation
 */
export type ToJSON<In, Out extends JSONUnknown = IntoJSON<In>> = Encoded<In, Out>;
export type IntoJSON<T> = T extends JSONScalar ? T : T extends {
    toJSON(): infer U;
} ? IntoJSON<U> : T extends Array<infer U> ? IntoJSON<U>[] : T extends JSONObject ? IntoJSONObject<T> : never;
